        ...
        private void btn_buscar_codigo_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.txt_codigo.Text != "")
                {
                    ...
                    this.dat_resultado.DataSource = Negocios.Mostrar().Where(x => x.ID_Medicamento == Codigo).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        private void dat_resultado_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        ID_Medicamento = int.Parse(this.dat_resultado.Rows[e.RowIndex].Cells[0].Value.ToString()),
        ...
        private void lst_productos_ItemChecked(object sender, ItemCheckedEventArgs e)
        {
            try
            {
                foreach (ListViewItem lista in lst_productos.CheckedItems)
                {
                    ...
                    var Prod = Productos.Where(x => x.ID_Medicamento == IDProducto).FirstOrDefault();
                    ...
                    using (TransactionScope Ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                    {
                        try
                        {
                            foreach (var Item in Productos)
                            {
                                ...
                                Negocios.Afectar_Inventario(Item.ID_Medicamento);
                                linea++;
                            }
                            Ts.Complete();
                        }
