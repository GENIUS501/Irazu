...
namespace Lienzos
{
    public partial class ListaTipoMedicamento : Form
    {
        int valorcelda = -1;
        public int Usuario { get; set; }
        public int Id_Rol { get; set; }
        public ListaTipoMedicamento()
        {
            InitializeComponent();
        }

        private void ListarTipoMedicamentos_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void dat_principal_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    ListarTipoMedicamentos_Load(null, null);
                }
                else
                {
        ...
        private void btn_buscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    NTipo_Medicamentos Negocios = new NTipo_Medicamentos();
                    this.dat_principal.DataSource = Negocios.Mostrar().Where(x => x.ID_Tipo_Medicamento == int.Parse(this.txt_buscar_cedula.Text)).ToList();
                }
            }
            catch (Exception ex)
            {
