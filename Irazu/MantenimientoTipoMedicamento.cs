...
namespace Lienzos
{
    public partial class MantenimientoTipoMedicamento : Form
    {
        public int Usuario { get; set; }
        public string Accion { get; set; }
        public int Id { get; set; }
        public MantenimientoTipoMedicamento()
        {
            InitializeComponent();
        }
        private void llenar()
        {
            NTipo_Medicamentos Negocios = new NTipo_Medicamentos();
            ETipo_Medicamentos Obj = new ETipo_Medicamentos();
            Obj = Negocios.Mostrar().Where(x => x.ID_Tipo_Medicamento == Id).FirstOrDefault();
            this.txt_id.Enabled = false;
            this.txt_id.Text = Obj.ID_Tipo_Medicamento.ToString();
            ...
            try
            {
                ...
                if (!validar())
                {
                    if (Accion == "A" || Accion == "M")
                    {
                        ...
                        if (Accion == "M")
                        {
                            Obj.ID_Tipo_Medicamento = int.Parse(this.txt_id.Text);
        ...
        private void MantenimientoTipoMedicamento_Load(object sender, EventArgs e)
        {
            try
            {
                if (Accion == "A")
